...
// and they have no associated DataFormat. GetDataType maps a Go string to
// TypeString (not a json type); the json distinction is a schema-level
// declaration, so a raw Go string is never auto-detected as json.
...
// json types have no DataFormat, so no format maps back to them.
func TestNoFormatMapsToJsonTypes(t *testing.T) {
	for format, dataType := range TypeFormatMap {
		if dataType == TypeJsonObject || dataType == TypeJsonArray {
			t.Fatalf("format %q unexpectedly maps to json type %q; json is type-only", format, dataType)
		}
	}
}
